...
public class CodeGenerator {
    ...
    public void semanticFunction(int func, Token next) {
        LogUtils.print("codegenerator : " + func);
        switch (func) {
            case 0:
                return;
            case 1:
...
//    }
    ...
    public void call() {
        ...
        try {
            ...
            ErrorHandlerUtils.printError("The few argument pass for method");
...
//        String className = symbolStack.pop();
        try {
            ...
            if (param.varType != t) {
                ErrorHandlerUtils.printError("The argument type isn't match");
            }
...
//        symbolStack.push(className);
            ...
            ErrorHandlerUtils.printError("Too many arguments pass for method");
        }
        callStack.push(methodName);
...
//        try {
            if (s1.varType != s2.varType) {
                ErrorHandlerUtils.printError("The type of operands in assign is different ");
            }
...
//        }
    ...
    public void equal() {
        ...
        Address s1 = ss.pop();
        if (s1.varType != s2.varType) {
            ErrorHandlerUtils.printError("The type of operands in equal operator is different");
        }
    ...
    public void lessThan() {
        ...
        Address s1 = ss.pop();
        if (s1.varType != VarType.Int || s2.varType != VarType.Int) {
            ErrorHandlerUtils.printError("The type of operands in less than operator is different");
        }
    ...
    public void and() {
        ...
        if (s1.varType != VarType.Bool || s2.varType != VarType.Bool) {
            ErrorHandlerUtils.printError("In and operator the operands must be boolean");
        }
    ...
    public void not() {
        ...
        if (s1.varType != VarType.Bool) {
            ErrorHandlerUtils.printError("In not operator the operand must be boolean");
        }
    ...
    public void methodReturn() {
        ...
        if (s.varType != temp) {
            ErrorHandlerUtils.printError("The type of method and return address was not match");
        }
